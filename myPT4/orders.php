...
<head>
  ...
  <title>Meat Factory : Orders</title>
<!-- Bootstrap -->
    ...
    <div class="col-xs-12 col-sm-8 col-sm-offset-2 col-md-6 col-md-offset-3">
     ...
     <form action="orders.php" method="post" class="form-horizontal">
      ...
      <div class="form-group">
          ...
          <div class="col-sm-9"> 
            <input name="order date" type="text" class="form-control" id="orderdate" placeholder="Order Date" readonly value="<?php if(isset($_GET['edit'])) echo $editrow['fld_order_date']; ?>" required>
        </div>
        </div>
        ...
        <div class="form-group">
          ...
          <select name="cid" class="form-control" id="customer" required>
            ...
            try {
              $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
              $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $stmt = $conn->prepare("SELECT * FROM tbl_customers_a179904_pt2");
              $stmt->execute();
              $result = $stmt->fetchAll();
            }
            catch(PDOException $e){
  ...
  <div class="row">
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      ...
      try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "SELECT * FROM tbl_orders_a179904_pt2, tbl_staffs_a179904_pt2, tbl_customers_a179904_pt2 WHERE ";
            $sql = $sql."tbl_orders_a179904_pt2.fld_staff_num = tbl_staffs_a179904_pt2.fld_staff_num and ";
            $sql = $sql."tbl_orders_a179904_pt2.fld_customer_num = tbl_customers_a179904_pt2.fld_customer_num";
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM tbl_orders_a179904_pt2");
            $stmt->execute();
            $result = $stmt->fetchAll();
